...
#   ./scripts/nemotron.sh logs [SERVICE]      View logs (asr, llm, or all)
...
#   ./scripts/nemotron.sh start --no-llm    # ASR only
...
# HuggingFace model cache path for ASR (auto-downloaded on first run)
...
Commands:
  ...
  logs [SERVICE]      View logs (asr, llm, or all)
...
cmd_start() {
    ...
    # If the ASR model is not cached, use a longer timeout for download
    MODELS_TO_DOWNLOAD=""
...
cmd_logs() {
    ...
    case "$SERVICE" in
        ...
        all)
            ...
            echo "  [ASR] = ASR service, [LLM] = LLM service"
            echo ""
        ...
        *)
            ...
            echo "Valid services: asr, llm, all"
            exit 1
            ;;
    esac
}
